...
| Electron GUI | ~76–85% | 壳 + 流式 + 权限 + runtime IPC/client + 会话切换/恢复 + composer controls + 多 provider；model/effort 与关键运行态事件待接 |
...
queue/steer/interrupt（`c08254a`）已收口，下一切片是 model/effort 设置。
