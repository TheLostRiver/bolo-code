...
| Electron GUI | ~72–82% | 壳 + 流式 + 权限 + runtime IPC/client + 会话切换/恢复 + Settings + 多 provider；composer controls 与 model/effort 待接 |
...
runtime 生产协议桥（`74997ab`）与会话切换/恢复（`c76123e`）已收口，
下一切片是 composer queue/steer/interrupt 与 model/effort。
