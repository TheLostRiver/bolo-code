...
| **斜杠** | **~85–91%** | OI-10 命令级发现/补全 + OI-12A argument hint 已完成；OI-15A–C 已建立完整 display policy、retained 单槽 primitive，并迁移只读 panel/pager；OI-15D–F 继续处理 Skills/Plugins、toast/error 与兼容桶 |
| **CLI TUI** | **~83–91%** | OI-14 retained renderer 主体与 OI-15A–C command surface 已完成；`/context`、`/doctor`、help/status 类结果已使用有界 panel/text pager；下一步 Skills/Plugins stable-key overlay、toast/error 与 compatibility cleanup；OI-H3 真人 Windows Terminal 仍未验 |
...
**当前默认 agent 队列：OI-15D → OI-15F。** OI-14 已建立单 retained
 renderer/OverlayHost，却仍保留一个 `compatibilityOutput` 字符串桶：尚未迁移的
slash 结果仍可能通过 `appendCompatibilityOutput()` 持续拼接并固定在 transcript
与 Composer 之间。OI-15A 已在 core 定义并运行时校验
...
compatibility bucket，plain/non-TTY 字节保持不变。OI-15D 起处理 Skills/Plugins 与
异步 stale guard；最后禁止 normal slash 输出
...
**agent 可闭环开放项：OI-15D–F。** OI-H1/H2/H3 是明确真人项，LSP 继续由证据
...
| **OI-15 · slash 命令 surface/lifecycle** | 🚧 A core display policy ✅（`d681734`）→ B retained 单 panel/toast slot ✅（`d6bd087`）→ C context/doctor/status 与只读 panel/pager ✅（`26f796f`）；下一步 D Skills/Plugins → E toast/error → F compatibility bucket 清理 |
...
但 OI-15 已确认 slash 查询/动作结果缺少 surface 与 lifecycle，当前默认队列按
packages-first 补齐 panel/toast/overlay/history、替换/清除/TTL/pager 和 stale guard；
外部资源与真人验收继续单列。

**当前推进：** OI-15D。OI-15C `26f796f` 已让 retained CLI 消费 resolved
panel/pager policy：`/context` 使用 12 秒无嵌套 compact dashboard，details/detail/
`--details` 使用 text pager；`/doctor`/`/status` 与 help/memory 等只读命令按真实
viewport 在短 panel 与长 pager 间切换，mcp/hooks 直接使用 pager。20 次重复 context
只替换单槽，迁移命令的 compatibility/plain writer 为 0，结果不进入 session
messages；plain/non-TTY fallback 保持原字节。下一刀为 Skills/Plugins stable-key
overlay、loading→result replace 与 stale async guard。真人 Windows Terminal 验收仍单列，
不用自动门禁冒充主观观感。
中段压缩与远端压缩按证据门控**显式关闭**
...
| 30 | **OI-15 · slash 命令 surface/lifecycle** | A core display policy ✅ `d681734` · B retained panel/toast 单槽 ✅ `d6bd087` · C context/doctor/status 与只读 panel/pager ✅ `26f796f` · D Skills/Plugins overlay · E toast/error policy · F compatibility cleanup | `/context` 位于 Composer 下方并自动清除；重复查询 replace；长内容 pager；短动作 toast；迟到结果不覆盖当前视图 | core exhaustive + 20×重复查询 + fake clock/generation + single/multi-page/resize/focus/reset + persistence/plain/JSON + full/dist/pack/install | 🚧 A/B/C 已关闭；下一刀 OI-15D |
