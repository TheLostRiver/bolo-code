...
| 35 | **ENH · Agent 面强化计划（本地优先）** | 见 [AGENT_ENHANCEMENT_PLAN.md](./AGENT_ENHANCEMENT_PLAN.md)：ROB-1 工具重复检测 ✅ 本轮 · ROB-2 悬空 tool call 修复 · CMP-1 压缩专用模型/墙钟预算 · ROB-3 后台任务 manifest · TERM-1 终端能力探测 · TERM-2 CSI 分片重组 · HKP-1 hooks 事件面 · HKP-2 auto 命令级安全分析 · HKP-3 plan 正交 · REN-1 fidelity 自检 · MEM-1 跨会话记忆 MVP · CMP-2 预压缩 · MEM-2 检索质量链 · TERM-3 滚轮规范化 · CBG-1 符号索引懒启动 · CMP-3 Segments 压缩 · REN-2 checkpoint 渲染 · REN-3 子进程隔离渲染 · EVT-1 事件总线 · WT-1 worktree 快照/GC · HKP-4 变更归因 | 按成本低→高排序实施，先红灯后实现 | 每项定向 + typecheck + 完整 `npm test` | **▶ NEXT：ROB-2**；ROB-1 已交付（shared 指纹契约 + queryLoop 提醒/中止）；依赖项：MEM-2←MEM-1、TERM-3←OUT-4、CMP-3 默认关闭 |
