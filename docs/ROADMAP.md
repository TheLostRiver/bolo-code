...
| **Electron GUI** | **~72–82%** | 壳 + 流式 + 权限 + 设置 + 多 provider（CX7）+ runtime v1 + **会话切换/恢复** + AskUserQuestion；composer controls / model-effort 待接，真人点击仍未验 |
...
| AR3 Desktop shell | ⏳ runtime 生产桥/会话切换恢复/视图模型/NSIS 已有，composer 与 model/effort 设置仍在 OI-06 |
...
runtime 生产桥与会话切换/恢复已完成，下一切片接 composer
queue/steer/interrupt 与 model/effort 设置，随后补齐运行态事件投影。
OI-04 已以
...
| 18 | **AR3A–F** | A ✅ client/store + core adapter + 生产 IPC · B/C ✅ 会话切换恢复/视图模型/薄壳 · F ✅ NSIS；D/E 待接 | Codex App 风格 Desktop（[设计方案](./DESKTOP_DESIGN.md)） | mock/core IPC + crash/restart + Windows package | ⏳ **OI-06 当前开放**。真实 Electron 已完成 runtime hello/query 与自动化 session-row click/resume；下一刀 composer controls 与 model/effort。⚠️ 真人点击与窗口视觉仍未验 |
