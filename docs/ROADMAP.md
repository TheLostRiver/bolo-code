...
| **CLI TUI** | **~58–68%** | slash/context/paste/Thought/权限等业务能力已存在；OI-14A/B/C 已完成真实 VT/选型、纯 live view-state 与 opt-in retained 基座，当前进入 OI-14D transcript/Markdown |
...
**已闭环主线：** headless 日用 → Diff（D0–D7 / U0–U4）· Hooks（H0–H5）· Compact（C0–C5）· Provider（P0–P4.1）· Effort（E0–E9）· Provider UX（CX0–CX8）· 可靠性（R0–R4）· **Durable Runtime（DR0–DR4）** · **Autonomous Road AR1 CLI/TUI runtime UX** · **AR3 Desktop 产品接线** · **OI-07 SearXNG 上游诊断、`search doctor` 与可选 Docker setup** · **OI-08B CLI 零步骤首次启动** · **OI-14A 真实 VT/renderer 选型** · **OI-14B live view-state** · **OI-14C retained renderer 基座**。OI-09–OI-13 的 slash/context/paste/Thought/权限/welcome 等局部切片保留为完成历史，但后续真实截图已证伪“整个 TUI renderer 稳定”的口径，系统性主线继续推进 **OI-14D-H**。切片明细 → [ROADMAP_HISTORY.md](./ROADMAP_HISTORY.md)。

**当前默认 agent 队列：OI-14D。** OI-14C `1798a7c` 已建立单一 Bolo terminal
adapter、稳定 retained 根树、theme/width/resize、水晶 welcome 与 session 级 engine
选择；它只消费 OI-14B `CliTuiViewState`，24/38/56/80/120/160/220 列真实 VT、
resize、new/resume、single-writer、plain byte snapshot、dist install、Electron
launch 与 126 脚本完整门禁全绿。retained 在 OI-14G 前仍只通过
`BOLO_TUI_ENGINE=retained` 显式 opt-in，默认 legacy 的正文/间距故障仍在。下一刀
迁移 transcript 与成熟 Markdown。完整方案见
...
**agent 可闭环开放项：OI-14D–OI-14H。** OI-13A–OI-13D（含 B2 回归）的局部
...
| **OI-14 · CLI TUI retained renderer 重构** | ▶ A 已关闭：真实 VT 四项红灯 + Pi direct/Node/adapter 选型；当前 B live view-state，C-H 待推进 |
...
**一句话：** Headless 核心主路径已收口；OI-14A/B/C 已完成真实终端证据、retained
选型、纯 live view-state 与 opt-in renderer 基座，默认 agent 队列为 OI-14D
transcript/Markdown；外部资源和真人验收继续单列。

**最近闭环：** OI-14C。`1798a7c` 建立 Bolo terminal adapter、稳定 retained root、
theme/viewport/resize、水晶 welcome、engine 路由和 async legacy panel bridge；
24–220 列真实 VT、single-writer、scrollback、new/resume、plain byte snapshot 与
126 项完整门禁全绿，单文件产物为 1,518,187 bytes。**当前实施：** OI-14D 迁移
transcript 与 Markdown；默认仍为 legacy，真人 Windows Terminal 验收仍单列，已知
auto-wrap/cursor/layout 缺陷不冒充人工项。
中段压缩与远端压缩按证据门控**显式关闭**
...
| 面 | 落点 |
|----|------|
| 构建 | `scripts/build-dist.ts`：esbuild bundle → `dist/bolo.mjs`（1,518,187 bytes / 185 模块）+ 拷 `bundled-skills` |
