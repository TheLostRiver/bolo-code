...
| **Agent 能力面（工具集）** | **~82–88%** | 15 个常驻/可选工具 + 显式 SearXNG `WebSearch`；五条外部线路已活体验证，SearXNG 仅本地 fixture：Bash（含 `run_in_background`）· BashOutput · KillShell · Read/Write/Edit/apply_patch · Glob/Grep · Skill · WebFetch · Agent · **TodoWrite** · **ExitPlanMode** · **AskUserQuestion**；见 §14 |
...
| **Electron GUI** | **~76–85%** | 壳 + 流式 + 权限 + 多 provider（CX7）+ runtime v1 + **会话切换/恢复 + composer controls** + AskUserQuestion；model/effort 与关键运行态事件待接，真人点击仍未验 |
...
| AR3 Desktop shell | ⏳ runtime 生产桥/会话切换恢复/视图模型/composer/NSIS 已有，model/effort 与关键运行态事件仍在 OI-06 |
...
runtime 生产桥、会话切换/恢复与 composer queue/steer/interrupt 已完成，
下一切片接 model/effort 设置，随后补齐关键运行态事件投影。
OI-04 已以
...
| 18 | **AR3A–F** | A ✅ client/store + core adapter + 生产 IPC · B/C ✅ 会话切换恢复/视图模型/薄壳 · D ✅ composer · F ✅ NSIS；E 待接 | Codex App 风格 Desktop（[设计方案](./DESKTOP_DESIGN.md)） | mock/core IPC + crash/restart + Windows package | ⏳ **OI-06 当前开放**。真实 Electron 已完成 runtime hello/query 与自动化 session-row click/resume；下一刀 model/effort，再补关键运行态事件。⚠️ 真人点击与窗口视觉仍未验 |
...
| **真·本地搜索路径** | ✅ OI-04 已实现 | [LOCAL_SEARCH_AND_FETCH.md](./LOCAL_SEARCH_AND_FETCH.md)：SearXNG compose（默认只开 `html`，须显式加 `json`）+ `search.searxng` 配置 + endpoint/预算/隐私边界。Bolo 已删除虚构桥 preset，零依赖直连 JSON API；本地 fixture 覆盖协议和生产接线，真实实例 live smoke 仍为 OI-X1。SearXNG 是本地服务但不是本地索引，查询通常仍到上游引擎 |
