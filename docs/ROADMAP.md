...
| **斜杠** | **~87–93%** | OI-10 命令级发现/补全 + OI-12A argument hint 已完成；OI-15A–E 已建立完整 display policy、retained 单槽 primitive，并迁移只读 panel/pager、Skills/Plugins catalog overlay 与 toast/error；OI-15F 继续清理兼容桶 |
| **CLI TUI** | **~85–92%** | OI-14 retained renderer 主体与 OI-15A–E command surface 已完成；只读状态使用有界 panel/text pager，扩展目录使用 stable-key overlay，短反馈使用 footer toast；下一步 compatibility cleanup；OI-H3 真人 Windows Terminal 仍未验 |
...
**当前默认 agent 队列：OI-15F。** OI-14 已建立单 retained
...
不写 compatibility/plain writer/session messages。OI-15F 最后禁止 normal slash 输出
...
**agent 可闭环开放项：OI-15F。** OI-H1/H2/H3 是明确真人项，LSP 继续由证据
...
| **OI-15 · slash 命令 surface/lifecycle** | 🚧 A core display policy ✅（`d681734`）→ B retained 单 panel/toast slot ✅（`d6bd087`）→ C context/doctor/status 与只读 panel/pager ✅（`26f796f`）→ D Skills/Plugins stable-key overlay ✅（`21ee1e2` / `87054df`）→ E toast/error policy ✅（`1d49d53`）；下一步 F compatibility bucket 清理 |
...
**当前推进：** OI-15F。OI-15E `1d49d53` 已让 retained CLI 消费 toast/history：
...
保持原字节，完整 `npm test`、dist/install、预算与 Electron smoke 全绿且没有新增依赖。
OI-15F compatibility cleanup 仍开放。真人 Windows Terminal 验收仍单列，
不用自动门禁冒充主观观感。
中段压缩与远端压缩按证据门控**显式关闭**
