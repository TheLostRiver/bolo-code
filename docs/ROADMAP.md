...
| **Subagent（对照 HC AgentTool）** | **~40%** | S0–S6 最小完成线：真 `runSubagent` + Agent 工具；S7+ 目录/侧链/fork 未做 |
...
**Bolo 现状一句话：** S0–S6 已落地真 loop + Agent 工具；S7+（目录定义 / 侧链 / fork）未做。
...
M-Cost：**C1–C4 已完成**（`PROMPT_CACHE.md`、stable/volatile、`test-prompt-cache`）；C5+（cache 请求头、tool 结果预算等）仍后置。
...
| **Subagent 真 loop / Agent 工具** | ✅ S0–S6；S7+ 未做 |
| Prompt cache 布局 | ⬜ |
...
| `PROMPT_CACHE.md` | 静态/动态边界与 cache 友好前缀（C1–C4） |
...
| **M-Subagent** | 🟡 | **S0–S6 最小完成线 ✅；S7+ 目录/侧链/fork 未做** |
