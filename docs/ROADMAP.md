...
| **CLI TUI** | **~90–95%** | OI-09–OI-11 已收口：水晶欢迎页 · 常驻全宽 composer · slash 发现/补全 · gutter/用户块 · 分段 Thinking/`Thought for` · model/token/keys footer · 三态权限详情 · 局部面板重绘 · Responses abort 诊断；真人 Windows Terminal 观感仍单列 |
...
**当前默认 agent 队列：为空。** OI-11 于 2026-07-28 完成 A–H：Responses timeout
分源、常驻 composer、时间线/status、分段活动、权限详情选择、局部 viewport 重绘、
水晶身份和文档均已进入 121 项完整门禁。真实终端仍只保留不可由自动化替代的 OI-H3。

**外部或人工阻塞项单列，不与 agent 队列混淆：**

| 待办 | 卡在哪 |
|------|--------|
| CLI TUI 真实 Windows Terminal 观感/光标/按键 | OI-11 已修完所有已知可自动闭环缺陷；只剩真人确认实际字体下的光标、颜色、动画、resize 与按键观感 |
...
**agent 可闭环开放项：** 当前为空；关闭证据见 [OPEN_ISSUES.md](./OPEN_ISSUES.md)。
CLI init 不再是默认安装步骤，SearXNG Docker 管理也只是显式可选能力；真人验收
状态仍单列，不从 `BLOCKED: HUMAN` 反推新功能。
...
**一句话：** 核心主路径与 OI-11 CLI TUI 已收口；默认 agent 队列为空，外部资源和
真人验收继续单列。
