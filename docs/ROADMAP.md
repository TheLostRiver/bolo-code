...
| **斜杠** | **~87–93%** | OI-10 命令级发现/补全 + OI-12A argument hint 已完成；OI-15A–D 已建立完整 display policy、retained 单槽 primitive，并迁移只读 panel/pager 与 Skills/Plugins catalog overlay；OI-15E–F 继续处理 toast/error 与兼容桶 |
| **CLI TUI** | **~85–92%** | OI-14 retained renderer 主体与 OI-15A–D command surface 已完成；只读状态使用有界 panel/text pager，扩展目录使用 stable-key overlay；下一步 toast/error 与 compatibility cleanup；OI-H3 真人 Windows Terminal 仍未验 |
...
**当前默认 agent 队列：OI-15E → OI-15F。** OI-14 已建立单 retained
...
`PgUp`/`PgDn`/`Home`/`End`。OI-15E 处理动作 toast/error；最后禁止 normal slash 输出
...
**agent 可闭环开放项：OI-15E–F。** OI-H1/H2/H3 是明确真人项，LSP 继续由证据
...
| **OI-15 · slash 命令 surface/lifecycle** | 🚧 A core display policy ✅（`d681734`）→ B retained 单 panel/toast slot ✅（`d6bd087`）→ C context/doctor/status 与只读 panel/pager ✅（`26f796f`）→ D Skills/Plugins stable-key overlay ✅（`21ee1e2` / `87054df`）；下一步 E toast/error → F compatibility bucket 清理 |
...
**当前推进：** OI-15E。OI-15D `21ee1e2` / `87054df` 已让 Skills/Plugins、
commands、market/search 使用 core pre-dispatch preview 和结构化 catalog，在唯一
OverlayHost 内完成 loading→result 原位替换。20 次 `/skills` stable-key 调用不写
compatibility/plain writer/session messages；`key + generation + sessionId + cwd`
阻止迟到结果覆盖当前视图；40 项目录在 18→10 行 resize 后仍有界并保持选中项可见，
支持 `PgUp`/`PgDn`/`Home`/`End`，关闭后恢复 Composer value/cursor/focus。
plain/non-TTY fallback 保持原字节且没有新增依赖。下一刀为动作反馈 toast/error
分级；OI-15F compatibility cleanup 仍开放。真人 Windows Terminal 验收仍单列，
不用自动门禁冒充主观观感。
中段压缩与远端压缩按证据门控**显式关闭**
