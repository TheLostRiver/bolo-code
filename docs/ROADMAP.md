...
| **CLI TUI** | **~89–95%** | OI-14 retained renderer、OI-15A–F command surface/lifecycle、OI-16/17 pager 布局与 **OUT-1 ToolPresentation ✅ `78ad65a`、OUT-2 默认折叠/键盘预览 ✅ `ab8a634`、OUT-3 按需全文/恢复 ✅ `595b172`** 已完成；OUT-4/5 鼠标与聚合待实现；OI-H3 真人 Windows Terminal 主观观感仍未验 |
...
delete consumer 尚不存在；鼠标和相邻只读调用聚合继续分属 OUT-4/5。OI-15A–F、
...
**agent 可闭环开放项：OUT-4..5。** OI-H1/H2/H3 是明确真人项，LSP
...
| 34 | **OUT-1..5 · 长工具输出摘要/折叠/全文查看** | 1 shared ToolPresentation ✅ `78ad65a` · 2 renderer-local fold/键盘 preview ✅ `ab8a634` · 3 file pager/resume ✅ `595b172` · 4 SGR mouse · 5 连续只读调用聚合 | 成功长输出默认有界摘要；运行 tail/错误 cap；有效 ref 按需查看全文并恢复 Composer；plain 不变 | long/ANSI/CJK/resize/stream/resume/spill/path safety + xterm mouse/paste/focus + perf/full/dist | **▶ NEXT：OUT-4**；OUT-3 已完成惰性全文分页、坏 ref 降级与 resume side-channel；cleanup primitive 已有但尚无 session delete consumer |
