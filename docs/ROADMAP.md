...
| **CLI TUI** | **~89–95%** | OI-14 retained renderer、OI-15A–F command surface/lifecycle、OI-16/17 pager 布局与 **OUT-1 ToolPresentation ✅ `78ad65a`、OUT-2 默认折叠/键盘预览 ✅ `ab8a634`、OUT-3 按需全文/恢复 ✅ `595b172`、OUT-4 SGR 鼠标 ✅ 本轮、OUT-5 只读调用聚合 ✅ 本轮** 已完成；OI-H3 真人 Windows Terminal 主观观感仍未验 |
...
`/tools` 最近工具 picker。OUT-3 `595b172` 已对有效受控 ref 按需分页读取 spill 全文，
缺失/损坏/无有效 ref 时明确回退 bounded preview；`tool_presentation` transcript
side-channel 可跨 compact/resume 恢复。安全 cleanup primitive 已完成，但正式 session
delete consumer 尚不存在。OUT-4 已自研 SGR 1006 鼠标：shared 纯解析、
adapter 与 raw input 生命周期绑定的 enable/disable（`TERM=dumb`/能力不足不启用）、
retained render 的 overflow block hit region、点击打开/关闭/切换 tool pager
（受控 ref 走惰性全文，否则 bounded preview）；wheel/release 只解析无行为，
键盘路径等价可用，真人 Windows Terminal 手感仍属 OI-H3。OUT-5 已把相邻只读调用
（Read/Grep/Glob 等 read/search 分类的终态成功块）聚合成 `⇅ N read-only calls`
组展示：成员固定单行 summary、不参与 `Ctrl+O` 展开、点击成员打开该成员 pager；
写工具/Bash/mcp/正文/错误/有文本思考/running/失败切断组，空思考占位被吸收；
MCP 只读性无法由名字判定故不聚合。相邻只读调用聚合属 OUT-5。
OI-15A–F、
...
补空到 45 个组件行，裁掉 footer 并挤压 Composer。OI-16 `5b22c15` 将 text pager
正文限制为 `min(18, max(1, rows - 6))`，短内容按实际行数渲染；24/48/80 行的
host 组件高度、两页导航与 plain 字节稳定进入默认门禁。但该测试未经过 Pi composite，
因此没有覆盖底部绝对锚定坐标。

第二张真人截图证明 OI-16 只生效一半：Doctor 已是 18 行正文和 `1/2` footer，但仍
固定在终端底部，与顶部 Composer 相隔大面积空白。OI-17 `cda22fd` 让同一 host
提供 embedded/modal 两个无状态视图：REPL text/runtime pager 进入 Composer 邻接
根布局，Permission/Question/Picker/Catalog/Diff 继续走 modal，`rootVisible: false`
的顶层 runtime pager仍全屏。48/80 行真实 xterm composite、`q` 后草稿续写、modal/
ownership/reliability/runtime 回归与完整 `npm test` 均已通过。
...
| [TOOL_OUTPUT_AND_MODEL_CONTEXT_DESIGN.md](./TOOL_OUTPUT_AND_MODEL_CONTEXT_DESIGN.md) | **CTX/OUT 设计方案**（model limits · resolved metadata · Tool presentation · 折叠/pager/mouse） |
...
| 34 | **OUT-1..5 · 长工具输出摘要/折叠/全文查看** | 1 shared ToolPresentation ✅ `78ad65a` · 2 renderer-local fold/键盘 preview ✅ `ab8a634` · 3 file pager/resume ✅ `595b172` · 4 SGR mouse ✅ 本轮 · 5 连续只读调用聚合 ✅ 本轮 | 成功长输出默认有界摘要；运行 tail/错误 cap；有效 ref 按需查看全文并恢复 Composer；点击摘要开/关/切 pager；相邻只读调用聚合为组；plain 不变 | long/ANSI/CJK/resize/stream/resume/spill/path safety + xterm mouse/paste/focus + perf/full/dist | **✅ 全轨完成**；OUT-5 已聚合 read/search 类相邻调用为 `⇅ N read-only calls` 组（成员固定 summary、点击成员开 pager、空思考占位吸收）；wheel 只解析无行为、真人手感未验；cleanup primitive 已有但尚无 session delete consumer |
| 35 | **ENH · Agent 面强化计划（本地优先）** | 见 [AGENT_ENHANCEMENT_PLAN.md](./AGENT_ENHANCEMENT_PLAN.md)：ROB-1 工具重复检测 ✅ · ROB-2 悬空 tool call 修复 ✅ · CMP-1 压缩专用模型/墙钟预算 ✅ · ROB-3 后台任务 manifest ✅ · TERM-1 终端能力探测 · TERM-2 CSI 分片重组 · HKP-1 hooks 事件面 · HKP-2 auto 命令级安全分析 · HKP-3 plan 正交 · REN-1 fidelity 自检 · MEM-1 跨会话记忆 MVP · CMP-2 预压缩 · MEM-2 检索质量链 · TERM-3 滚轮规范化 · CBG-1 符号索引懒启动 · CMP-3 Segments 压缩 · REN-2 checkpoint 渲染 · REN-3 子进程隔离渲染 · EVT-1 事件总线 · WT-1 worktree 快照/GC · HKP-4 变更归因 | 按成本低→高排序实施，先红灯后实现 | 每项定向 + typecheck + 完整 `npm test` | **▶ NEXT：TERM-1**；ROB-1/ROB-2/CMP-1/ROB-3 已交付（指纹守卫 + 消息配对修复 + 压缩模型/预算 + 后台任务 manifest 与重启提醒）；依赖项：MEM-2←MEM-1、TERM-3←OUT-4、CMP-3 默认关闭 |
