...
| **CLI TUI** | **~82–90%** | OI-14 retained 主体与 OI-15A–B display policy/单槽 command surface 已完成；OI-15C–F 迁移具体命令生命周期；真人 Windows Terminal 仍未验 |
...
**当前默认 agent 队列是 OI-15C → OI-15F。** 真人走查与代码审计确认普通 slash
...
generation guard。下一刀 OI-15C 将 `/context`、`/doctor` 和只读 status 映射到
panel/pager，之后处理 stable-key Overlay 与 compatibility cleanup。plain/non-TTY
...
→ OI-15C–F slash command migration/lifecycle 🚧
...
| **OI-15 · IN PROGRESS** | slash 结果 surface/lifecycle：A core display policy ✅（`d681734`）→ B retained single-slot panel/toast ✅（`d6bd087`）→ C context/doctor/status → D Skills/Plugins overlay → E toast/error policy → F compatibility cleanup；下一刀 OI-15C |
