...
一律从 `piCompat.ts` 导入；`terminal-image.js` 由本地 stub（`piTerminalImageStub.ts`）
替代——`piCompat` 源码层直连 stub（tsx 测试/构建同轨），pi-tui 内部相对导入由
`scripts/build-dist.ts` 的 esbuild plugin 在构建期拦截替换；stub 行为由
`scripts/test-pi-terminal-image-stub.ts` 单测守护。升级/替换/fork pi-tui 时只改
这两个文件。
...
| **CLI TUI** | **~89–95%** | OI-14 retained 主体、OI-15A–F slash lifecycle、OI-16/17 pager 布局与 OUT-1..3 ToolPresentation、默认折叠、键盘预览、按需全文与恢复、OUT-4 SGR 鼠标点击、OUT-5 只读调用聚合已完成 |
...
| 多 Provider 热切 | ~94–97% | **P0–P4.1 + CX7 Desktop**；CTX-1 config/resolver、CTX-2 runtime 单一消费链与 CTX-3 CLI/Desktop 元数据可观测性均已完成 |
...
provider output baseline 消费同一 metadata；CTX-3 `d966d4b` 已让 slash/doctor/model/
provider、CLI dashboard 与 Desktop header/settings 显示逐字段数值、来源和 warning。
OUT-1 `78ad65a` 已建立 Tool presentation 分类/validator、原始与保留规模、bounded
preview、session-scoped spill ref，以及 core event/CLI view-state 透传；模型 tool
message 不变。OUT-2 `ab8a634` 已建立 renderer-local stable block state，成功长结果
默认 summary，短结果/error 显示 bounded preview，running 显示有界 tail；`Ctrl+O`
只切换 summary/bounded preview。OUT-3 `595b172` 已让 retained-only `/tools` 对有效
ref 按需分页读取全文，缺失/损坏/无有效 ref 时回退 bounded preview，并通过
`tool_presentation` side-channel 跨 compact/resume 恢复。安全 cleanup primitive
已交付但没有正式 session delete consumer。OUT-4 已自研 SGR 1006 鼠标：shared 纯解析、
adapter 与 raw input 生命周期绑定的启用/禁用（`TERM=dumb`/能力不足不启用）、
retained render 的 overflow block hit region、点击打开/关闭/切换 tool pager（受控
ref 走惰性全文，否则 bounded preview）；wheel/release 只解析无行为，键盘路径等价
可用，真人 Windows Terminal 手感仍属 OI-H3。OUT-5 已把相邻只读调用聚合为
`⇅ N read-only calls` 组：成员固定单行 summary、不参与 `Ctrl+O` 展开、点击成员
打开该成员 pager；写工具/mcp/正文/错误/有文本思考/running/失败切断组，空思考
占位被吸收；MCP 只读性无法由名字判定故不聚合。正式契约见
[TOOL_OUTPUT_AND_MODEL_CONTEXT_DESIGN.md](./TOOL_OUTPUT_AND_MODEL_CONTEXT_DESIGN.md)；
CTX-1..3 与 OUT-1..5 已完成。
...
help/cost/memory/mcp/hooks 映射到 panel/text pager，并证明 20× replace、单页可见、
...
toast；迁移结果不写 compatibility/plain writer/session messages。OI-15F `d1e26bb`
已统一 Provider/Effort/Diff 的 action-picker/diff view，禁止 normal slash 命中
compatibility writer，并保留 visual-only/plain fallback。plain/non-TTY
...
OI-16 `5b22c15` 按第一张真人截图关闭了 OI-15C 漏验的 pager 组件高度：48/80 行
终端下 text pager 正文最多 18 行，29 行 Doctor 分成两页，短内容按实际高度显示。
但该专项只渲染 host，没有覆盖 Pi composite；第二张真人截图因此继续暴露了底部
绝对锚定在 Composer 与 pager 之间制造大面积空白。

OI-17 `cda22fd` 关闭了这个坐标缺口：同一 host 状态机通过 embedded/modal 两个
无状态 view 分别进入 retained 根布局与 Pi overlay 栈。REPL text/runtime pager
紧邻 Composer；Permission、Question、Picker、Catalog、Diff 仍为 modal，standalone
runtime pager仍全屏。48/80 行 xterm composite、`q` 后草稿续写、modal/runtime/
ownership/reliability 与完整门禁均已通过；plain/non-TTY 输出不变。
...
→ CTX-3 可观测性 / 文档 ✅ `d966d4b`
→ OUT-1 ToolPresentation 契约 ✅ `78ad65a`
→ OUT-2 默认折叠 / 键盘 preview ✅ `ab8a634`
→ OUT-3 file-backed pager / resume ✅ `595b172`
→ OUT-4 SGR mouse ▶ NEXT
→ OUT-5 连续只读调用聚合
...
| **OI-15 · CLOSED** | slash 结果 surface/lifecycle：A core display policy ✅（`d681734`）→ B retained single-slot panel/toast ✅（`d6bd087`）→ C context/doctor/status 与只读 panel/pager ✅（`26f796f`）→ D Skills/Plugins overlay ✅（`21ee1e2` / `87054df`）→ E toast/error policy ✅（`1d49d53`）→ F compatibility cleanup ✅（`d1e26bb`） |
