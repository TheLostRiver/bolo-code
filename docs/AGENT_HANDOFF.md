...
| **CLI TUI** | **~83–91%** | OI-14 retained 主体与 OI-15A–C display policy/单槽/只读 panel-pager 已完成；OI-15D–F 迁移 Skills/Plugins、toast/error 与 compatibility；真人 Windows Terminal 仍未验 |
...
**当前默认 agent 队列是 OI-15D → OI-15F。** 真人走查与代码审计确认普通 slash
...
resize/CJK、compatibility 与 session-message 隔离。下一刀处理 Skills/Plugins
stable-key Overlay，之后处理 toast/error 与 compatibility cleanup。plain/non-TTY
...
→ OI-15D–F Skills/Plugins · toast/error · compatibility cleanup 🚧
...
| **OI-15 · IN PROGRESS** | slash 结果 surface/lifecycle：A core display policy ✅（`d681734`）→ B retained single-slot panel/toast ✅（`d6bd087`）→ C context/doctor/status 与只读 panel/pager ✅（`26f796f`）→ D Skills/Plugins overlay → E toast/error policy → F compatibility cleanup；下一刀 OI-15D |
