...
| **CLI TUI** | **~88–94%** | OI-14 retained 主体、OI-15A–F slash lifecycle、OI-16/17 pager 布局与 OUT-1/2 ToolPresentation、默认折叠、键盘预览路径已完成；OUT-3..5 file-backed 全文、鼠标与聚合待实现 |
...
**当前 agent 可闭环队列：OUT-3..5；下一刀是 OUT-3。**
...
全局切换 summary/preview，retained-only `/tools` 用最近工具 picker 打开最多 4,000
字符的 embedded preview pager，关闭后恢复 Composer。OUT-3 再接 file-backed 全文
pager、resume side-channel 与 spill cleanup；鼠标和只读调用聚合分属 OUT-4/5。正式契约见
[TOOL_OUTPUT_AND_MODEL_CONTEXT_DESIGN.md](./TOOL_OUTPUT_AND_MODEL_CONTEXT_DESIGN.md)；
CTX-1..3 与 OUT-1..2 已完成，真正全文读取与恢复尚未实现。
...
→ OUT-3 file-backed pager / resume ▶ NEXT
→ OUT-4 SGR mouse
→ OUT-5 连续只读调用聚合
...
| **模型上下文 / 长工具输出** | **[TOOL_OUTPUT_AND_MODEL_CONTEXT_DESIGN.md](./TOOL_OUTPUT_AND_MODEL_CONTEXT_DESIGN.md)**（CTX-1..3、OUT-1..2 已完成 · OUT-3 下一刀） |
