...
SessionEvent
  ...
  → 常驻 Composer/activity/footer ▶ OI-14E NEXT
```
...
与 async legacy panel bridge。OI-14D 必须继续沿用 stable block id 和原始 source
text，不得在 Markdown renderer 中重建 stream/tool/search/resume 状态机。
...
| **CLI TUI** | **~64–74%** | slash/context/paste/Thought/权限/水晶等业务能力已存在；OI-14D 已在 opt-in retained 路径迁入稳定 transcript/Markdown，当前 OI-14E 迁 Composer/activity/footer；默认 legacy 仍保留已知布局故障 |
...
**已闭环：** Diff · Hooks · Compact（含 AR2 全段）· Provider · Effort · Provider UX CX0–CX8 · **CLI/Agent 可靠性 R0–R4** · **Durable Runtime DR0–DR4** · **Autonomous Road AR1 CLI/TUI runtime UX** · **AR-T1–T3+ Agent 能力面** · **AR3/OI-06 Desktop 产品接线** · **AR4 evidence gate** · **AR5 release hardening** · **OI-04 SearXNG 直连、OI-X1 真实实例 smoke、OI-07 上游诊断 / doctor / 可选 Docker setup、OI-08B CLI 零步骤首次启动、OI-14A 真实 VT/renderer 选型、OI-14B live view-state、OI-14C retained renderer 基座、OI-14D retained transcript/Markdown**。OI-09–OI-13 的局部 TUI 能力保留为完成历史，但不再代表 renderer 整体稳定。

**当前主线：OI-14E。** OI-14D `8b060e5` 已让 retained root 按 stable block id
原位渲染 User/Assistant/Thought/Tool/Search/Error/Warning/Summary，并接入 Pi
Markdown；父级统一拥有每条物理行的 gutter 与 section gap。whole/character/
fixed-random chunk、24–220 列、resize、resume、ANSI/OSC 8、CJK/emoji、
list/table/code、dist install、Electron launch 与 127 脚本完整门禁全绿；产物为
1,611,976 bytes / 189 modules。`BOLO_TUI_ENGINE=retained` 仍仅显式 opt-in，
Composer/activity/footer 与 overlays 尚未迁，默认 legacy 不变。后续固定顺序为
...
OI-14E 必须在 retained root 内建立常驻 Composer/activity/footer；不得通过修改
`retainedTranscript.ts` 的 gutter、section gap 或 block 几何来补输入区问题，也不得
新增第二个 stdout writer。slash/paste/history/composer 状态继续消费既有业务真源。
...
→ OI-14E Composer/activity/footer ▶ NEXT
```
...
| **OI-14 · OPEN** | retained renderer 重构：A 真实 VT/选型 ✅（`1ae9f53` / `f04f8de`）→ B live view-state ✅（`269b39c`）→ C renderer ✅（`1798a7c`）→ D Markdown/transcript ✅（`8b060e5`）→ E Composer → F overlays → G 默认切换 → H 删除 legacy；当前 OI-14E |
