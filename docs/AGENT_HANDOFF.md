...
SessionEvent
  ...
  → width-aware physical lines ✅ OI-14C 基座
...
| **CLI TUI** | **~58–68%** | slash/context/paste/Thought/权限/水晶等业务能力已存在；OI-14C 已建立 opt-in retained adapter/root/resize/single-writer 基座，当前 OI-14D 迁 transcript/Markdown；默认 legacy 的正文碎片、巨大空洞和续行贴左仍在 |
...
→ OI-14D transcript/Markdown ▶ NEXT
```
...
| **OI-14 · OPEN** | retained renderer 重构：A 真实 VT/选型 ✅（`1ae9f53` / `f04f8de`）→ B live view-state ✅（`269b39c`）→ C renderer ✅（`1798a7c`）→ D Markdown/transcript → E Composer → F overlays → G 默认切换 → H 删除 legacy；当前 OI-14D |
