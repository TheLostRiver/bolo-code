...
与 async legacy panel bridge。OI-14D `8b060e5` 已沿用 stable block id 和原始 source
...
| **CLI TUI** | **~68–78%** | OI-14A–E 已在 opt-in retained 路径迁入稳定 transcript/Markdown、常驻 Composer、分段 activity 与 footer；当前 OI-14F 迁 overlays，默认 legacy 仍保留已知布局故障 |
...
**已闭环：** Diff · Hooks · Compact（含 AR2 全段）· Provider · Effort · Provider UX CX0–CX8 · **CLI/Agent 可靠性 R0–R4** · **Durable Runtime DR0–DR4** · **Autonomous Road AR1 CLI/TUI runtime UX** · **AR-T1–T3+ Agent 能力面** · **AR3/OI-06 Desktop 产品接线** · **AR4 evidence gate** · **AR5 release hardening** · **OI-04 SearXNG 直连、OI-X1 真实实例 smoke、OI-07 上游诊断 / doctor / 可选 Docker setup、OI-08B CLI 零步骤首次启动、OI-14A 真实 VT/renderer 选型、OI-14B live view-state、OI-14C retained renderer 基座、OI-14D retained transcript/Markdown、OI-14E Composer/activity/footer**。OI-09–OI-13 的局部 TUI 能力保留为完成历史，但不再代表 renderer 整体稳定。

**当前主线：OI-14F。** OI-14E `d0fb822` 已让 retained root 常驻同一个
`RetainedComposer`，并接入 Bolo slash/hint/history/undo/footer、Pi
keys/StdinBuffer/硬件光标、分段 activity 与 silent/visible Thought 完成动作。
24–220 列、new/resume 真实 REPL、abort/raw-mode rollback、burst/resize/paste、
dist install、Electron launch 与 128 脚本完整门禁全绿；产物为 1,641,896 bytes /
192 modules。`BOLO_TUI_ENGINE=retained` 仍仅显式 opt-in，overlays 尚未迁，默认
legacy 不变。后续固定顺序为 overlays → 默认切换 → 删除 legacy。完整方案见
[CLI_TUI_REFACTOR_PLAN.md](./CLI_TUI_REFACTOR_PLAN.md)，选型数据见
[CLI_TUI_RENDERER_DECISION.md](./CLI_TUI_RENDERER_DECISION.md)。

OI-14F 必须在 retained root 内建立唯一 OverlayHost，把 permission/question/
provider/effort/diff/pager 的既有 view-model 组件化；不得重写权限/diff/question
业务状态机，也不得新增第二个 stdin/stdout owner。overlay 打开时保留同一 Composer
及 value/cursor/history/undo，关闭、Esc、Ctrl+C 或 abort 后确定恢复焦点。已迁面板
必须删除 `suspendForLegacyPanel()` 调用，剩余 bridge 调用使用静态白名单。
...
| **OI-14 · OPEN** | retained renderer 重构：A 真实 VT/选型 ✅（`1ae9f53` / `f04f8de`）→ B live view-state ✅（`269b39c`）→ C renderer ✅（`1798a7c`）→ D Markdown/transcript ✅（`8b060e5`）→ E Composer/activity/footer ✅（`d0fb822`）→ F overlays → G 默认切换 → H 删除 legacy；当前 OI-14F |
