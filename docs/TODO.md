...
> - **默认下一刀：M-GEN**（MCP 通用，`TODO_SKILL_MCP_PLUGIN.md` §5）  
> - **Skill S-PORT-0..8 ✅** 可移植最小出口（frontmatter·旁路·覆盖·矩阵·预算·creator·无远程）  
...
相对 HC headless **~40–55%**。Skill 可移植最小出口 ✅。默认下一刀：**M-GEN**（MCP）。