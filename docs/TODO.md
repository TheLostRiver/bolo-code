...
> 更新：**产品级下一轨 `docs/TODO_PRODUCT.md`**（整盘 headless；**非** Memory 窄轨）。  
> 历史：`TODO_AUTORUN.md` 余量小轨已圆满。Electron 仍 OUT。  
> 原则：无遥测；对照参考语义；状态按代码行为。
...
| **`TODO_PRODUCT.md`** | **整盘产品后续执行轨（当前真源；无 Electron）** |
| **`TODO_AUTORUN.md`** | 历史余量小轨（MEM/IMPORT/可观测）✅ 已收口 |
| **`TODO_MEMORY.md`** | 跨会话 Memory 专册 |
...
P1 主切片：~~MCP HTTP/SSE · PL2 · PL-MKT · Usage+ · RC* · STE · PERM · snip · J-D · auto Y0–Y4+Y3.6~~ ✅ 最小
  ...
  ~~AUTORUN 余量小轨~~ ✅（见 `TODO_AUTORUN.md`，**不是**整盘）
  **当前整盘下一轨：`docs/TODO_PRODUCT.md`（P-PL-ZIP → … → 收口；无 Electron）**
```

| 优先级 | 含义（当前） |
|--------|----------------|
| **P0** | 抬 headless 水位：韧性 / TP / CP 日用 已 🟡 |
| **P1** | **产品轨 `TODO_PRODUCT.md`**（zip 安装 · SA 并发帽 · doctor · 窄 TUI） |
| **P2** | 轨内未完成 |
...
下一阶段：
  ① ~~扩展三层 / Memory / AUTORUN 余量小轨~~ ✅
  ② **整盘产品轨** — `docs/TODO_PRODUCT.md`
       P-PL-ZIP → P-PL-URL-ZIP → P-SA-CAP → doctor/TUI 小步 → 收口
  ③ **OUT（另令）：** Electron · T8 完整 Ink · OAuth · 企业 YOLO · 官方市场 · worktree · cached MC
```
...
| **MEM-0…7** | **`TODO_MEMORY.md` / 跨会话 memory** |
| **P-PL-* · P-SA-* · P-T9-*** | **`TODO_PRODUCT.md` / 整盘产品轨** |
| MCP* · **PL-MKT** | M3（**PL-MKT 最小 ✅**；zip 加深在产品轨；官方市场 ⬜） |
| M4 / T8 | Electron / 完整 Ink ⬜（OUT） |
