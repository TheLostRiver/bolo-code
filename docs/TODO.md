...
> 更新：Memory MEM-0…5 ✅；**无人值守下一轨见 `docs/TODO_AUTORUN.md`（Electron 不在轨）**。  
...
| **`TODO_MEMORY.md`** | **跨会话 Memory（MEM-0…5 最小 ✅；MEM-6/7 在 AUTORUN）** |
| **`TODO_AUTORUN.md`** | **无人值守执行轨（A→E；无 Electron）** |
| `MCP.md` | MCP transport · 配置 · `/mcp` |
| `SKILLS.md` | Skill 目录 · catalog · Skill 工具 |
| `PLUGINS.md` | 插件 · PL-MKT 最小市场 |
| `MEMORY.md` | 跨会话 MEMORY.md 契约 · `/memory` |
...
P1 主切片：~~MCP HTTP/SSE · PL2 · PL-MKT · Usage+ · RC* · STE · PERM · snip · J-D · auto Y0–Y4+Y3.6~~ ✅ 最小
  ~~Skill→MCP→Plugin Spec~~ ✅ · ~~Memory MEM-0…5~~ ✅
  **下一轨：`docs/TODO_AUTORUN.md`（MEM-6… → IMPORT → SA → CP-OBS → 收口；无 Electron）**
```

| 优先级 | 含义（当前） |
|--------|----------------|
| **P0** | 抬 headless 水位：韧性 / TP / CP 日用 已 🟡 |
| **P1** | **AUTORUN 轨**（Memory 加深 · 只读导入 · SA/CP 可观测）；扩展三层 / auto / PL-MKT / MEM-0…5 已 ✅ |
| **P2** | 轨内未完成子项 |
| **P3** | Electron / 完整 Ink / OAuth / 企业 YOLO / worktree（**本轨 OUT**） |
...
下一阶段（**无人值守序 · 见 `TODO_AUTORUN.md`**）：
  ① ~~**Skill → MCP → Bolo 插件规范**~~ ✅（S-PORT · M-GEN · PL-SPEC 最小）
  ② ~~**跨会话 Memory 最小**~~ ✅（MEM-0…5）
  ③ **A** MEM-6 topic 扫描/相关 · MEM-7 project-scoped
  ④ **B** IMPORT-S1/P1/X（只读；非官方市场）
  ⑤ **C** SA-PAR/DOC · **D** CP-OBS/DOC · **E** DOC-SYNC + SMOKE
  ⑥ **OUT（本轨不做）：** Electron · T8 Ink · OAuth · 企业 YOLO · 官方市场 · worktree · MEM-8 team
```
...
> **执行轨真源：`docs/TODO_AUTORUN.md`**  
> - **启动后第一刀：`MEM-6`**（topic 扫描 + 确定性相关；无强制 side-LLM）  
> - 整轨：A Memory 加深 → B 只读导入 → C Subagent 可见 → D Compact 可观测 → E 收口  
> - **圆满：** AUTORUN §4 全 ✅；用户说「停下」则停  
> - 已齐：auto Y0–Y4+Y3.6 · PL-MKT · S-PORT/M-GEN/PL-SPEC · MEM-0…5  
> - 红线：无遥测；不接 Claude/Codex 官方市场；不擅自开 Electron/Y5+  
...
| **MEM-0…5 · MEM-6+** | **`TODO_MEMORY.md` / 跨会话 memory** |
...
**一句话：**  
auto / PL-MKT / 扩展三层 / Memory 最小已齐。  
**下一轨：`TODO_AUTORUN.md`（无 Electron）→ 从 MEM-6 起一路做到收口。**  
相对 HC headless **~40–55%**。