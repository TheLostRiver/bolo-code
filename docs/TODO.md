...
> 更新：**`TODO_AUTORUN.md` 轨已圆满**（MEM-6/7 · IMPORT · SA · CP-OBS）；Electron 仍 OUT。  
...
P1 主切片：~~MCP HTTP/SSE · PL2 · PL-MKT · Usage+ · RC* · STE · PERM · snip · J-D · auto Y0–Y4+Y3.6~~ ✅ 最小
  ~~Skill→MCP→Plugin Spec~~ ✅ · ~~Memory MEM-0…7~~ ✅ · ~~IMPORT 只读~~ ✅
  **~~AUTORUN 轨~~ ✅ 圆满**（见 `docs/TODO_AUTORUN.md`）
  **下一刀：仅 OUT 需新指令**（Electron · OAuth · 企业 YOLO · T8 · worktree …）
```

| 优先级 | 含义（当前） |
|--------|----------------|
| **P0** | 抬 headless 水位：韧性 / TP / CP 日用 已 🟡 |
| **P1** | ~~AUTORUN 轨~~ ✅；扩展三层 / auto / PL-MKT / Memory 加深 / 只读导入 已 ✅ |
| **P2** | 轨外余量（需确认） |
| **P3** | Electron / 完整 Ink / OAuth / 企业 YOLO / worktree（**OUT 残留**） |
...
下一阶段：
  ① ~~**Skill → MCP → Bolo 插件规范**~~ ✅
  ② ~~**跨会话 Memory 最小 + 加深**~~ ✅（MEM-0…7）
  ③ ~~**AUTORUN**~~ ✅（IMPORT · SA-PAR · CP-OBS · 收口）
  ④ **OUT 残留（需新指令）：** Electron · T8 Ink · OAuth · 企业 YOLO · 官方市场 · worktree · MEM-8 team · cached MC
```
...
> **`docs/TODO_AUTORUN.md` 已圆满收口。**  
> - 默认**无**强制下一刀；任选 OUT 需用户点名  
> - 已齐：auto · PL-MKT · S-PORT/M-GEN/PL-SPEC · MEM-0…7 · IMPORT 只读 · SA 可见 · CP-OBS  
...
**一句话：**  
auto / PL-MKT / 扩展三层 / Memory 加深 / 只读导入 / **AUTORUN 轨圆满**。  
相对 HC headless **~42–58%**。OUT 残留需新指令：Electron · OAuth · 企业 YOLO · T8 · worktree。