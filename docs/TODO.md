...
```text
主路径可跑（脚本/CLI）— 相对 HC headless ~45–60%（勿写 ~70% / 勿写「路线图完成」）。
...
粗估相对 HC：**~45–60%**。`TODO_PRODUCT` 收口 **≠** 整盘完成。
...
> **整盘真源：`docs/TODO_FULL.md`**  
> - **第一刀：`F-T8-INK`**（完整 Ink TUI 骨架 + plain 回退）  
> - 启动：`按 TODO_FULL 执行` / `开始整盘后续轨，直到圆满（不做 Electron）`  
> - `TODO_PRODUCT` 只是历史小轨，**不是**整盘完成  
> - 红线：无遥测；不接 Claude/Codex 官方市场；**不擅自做 Electron**
...
**一句话：**  
主路径可日用；相对 HC **~45–60%**。  
**下一轨：`TODO_FULL`（先 T8 Ink）。** Electron 不急；官方店/遥测永不做。