...
> - **默认下一刀：S-PORT-3/4**（覆盖序 · disable/user-invocable 钉死）或 **S-PORT-2**（旁路根）  
> - **S-PORT-1 ✅** frontmatter 契约（`packages/skills/src/frontmatter.ts`）  
...
相对 HC headless **~40–55%**。默认下一刀：**S-PORT-3/4**（Skill 行为钉死）。