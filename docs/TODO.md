...
| **`TODO_MEMORY.md`** | **跨会话 Memory（MEM-0…5 最小 ✅；topic/project 后置）** |
...
下一阶段：
  ...
  ② ~~**跨会话 Memory 最小**~~ ✅（MEM-0…5；`docs/TODO_MEMORY.md`）
  ③ 后置（需确认）：Memory topic/project · 官方市场深度 / MCP OAuth / OR6 / T8 / Electron / 企业 YOLO
```
...
> **Memory 最小已齐：** `docs/TODO_MEMORY.md` / `docs/MEMORY.md`（MEM-0…5）  
> - `~/.bolo/memory/MEMORY.md` 索引进 system（volatile · 行/字节 cap）  
> - `/memory` · `BOLO_MEMORY_DIR` / `BOLO_DISABLE_MEMORY`  
> - 后置：MEM-6 topic 相关检索 · MEM-7 project-scoped · team/daily  
> - 扩展三层：**S-PORT ✅** · **M-GEN-0..6+8 ✅** · **PL-SPEC ✅**  
...
**一句话：**  
auto / PL-MKT / 扩展三层 / **Memory 最小（MEM-0…5）** 已齐。  
相对 HC headless **~40–55%**。后置：Memory topic/project · OAuth/IMPORT/Electron。