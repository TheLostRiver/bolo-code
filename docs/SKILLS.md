...
| user | `~/.bolo/skills` | 盖 bundled |
| project | `.bolo/skills` | 盖 user |
| plugin | 插件 `skills/` | **最高** |

API：`mergeSkillsByPrecedence(...layers)`（后层赢）；`discoverSkills` = bundled→user→project；`loadWorkspace` 再 merge plugin。  
...
**后续：** S-PORT-2 旁路根（默认 off）· S-PORT-5 预算可观测 · S-PORT-6 creator — 见 **`docs/TODO_SKILL_MCP_PLUGIN.md`**。
