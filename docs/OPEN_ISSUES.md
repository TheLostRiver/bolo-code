...
- 准入时 `SlashDispatchResult` 只有 `ok`、`message` 和少数
  ...
  OI-15E 已迁移 toast/history 并建立显式 durable error 边界；OI-15F 已完成
...
- 准入时，`runOnePrompt()` 会把 slash 输入作为 typed user block 回显，所有普通
  ...
  该路径；OI-15F 已删除 normal slash 的兼容入口。
...
- Skills/Plugins catalog 在 slash handler 完成前用 core preview 打开 loading，完成后以
...
| **OI-15F · compatibility cleanup** | normal slash 禁止 `appendCompatibilityOutput`；迁移旧 `interactive*` | VT/plain/JSON/full test/dist/pack/install/owner guard 全绿 | **CLOSED · `d1e26bb`** |
