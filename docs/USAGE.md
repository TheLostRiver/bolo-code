...
bounded preview 间切换；输入 `/tools` 可按最近优先选择历史工具。Enter 对有效受控
...
| `/help` | 命令列表 |
| `/tools` | retained TTY 最近工具 picker；有效 ref 按需打开全文 pager，否则回退 bounded preview；Esc 返回原输入；打开/翻页不新增 session 或 provider messages |
