...
| Write | 全文写入；成功结果含行数 diff 摘要 + `meta`；ask 前可 preview |
| apply_patch | `*** Begin Patch`：Add / Update / Delete / **Move|Rename File**；或简易 unified；成功 `meta.kind=apply_patch` + `files[]`；会话 `fileDiffLog` + transcript `file_diff`；`/diff` · `/diff git` |
