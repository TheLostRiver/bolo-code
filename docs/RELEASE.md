...
| 指标 | 实测 | 预算（超出即需解释） |
|---|---|---|
| CLI 产物体积 | 1,803,844 bytes（OUT-2 `ab8a634`；较 1,385,065B 基线 +418,779B） | 基线 +1,500,000 bytes |
| CLI `--help` cold-start | OUT-2 完整门禁：增量 51.4ms | 增量 ≤ 100ms |
| retained 500 blocks / 10,000 行 | OUT-2 完整门禁：CPU 375.0ms · render heap +21.1MB · cleanup retained +1.5MB | CPU ≤ 3s · heap ≤ 128MB · cleanup ≤ 64MB |
...
| **长工具输出折叠/全文查看** | ⚠️ OUT-2 默认折叠/键盘预览完成，OUT-3 全文待实现 | `78ad65a` 已提供 bounded `ToolPresentation`、原始/保留规模、session-scoped spill ref 与 live event/view-state；`ab8a634` 已让成功长结果默认 summary、短结果/error 显示 bounded preview、running 显示有界 tail，并提供 `Ctrl+O` 与 retained-only `/tools`。当前 `/tools` pager 最多显示内存中的 4,000 字符 preview，尚不能从结构化 ref 按需读取 spill 全文；resume side-channel 与 spill cleanup 也仍待 OUT-3。模型 tool message、plain/non-TTY 与 session message 语义不变 |
