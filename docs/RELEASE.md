...
| **CLI TUI 的真实 Windows Terminal 呈现** | ❌ 未验证 | OI-09–OI-12 已自动覆盖水晶三档/ASCII/NO_COLOR、常驻全宽 composer、slash reducer/参数提示、context 仪表盘、bracketed-paste raw driver、CJK/emoji、响应式 gutter/dock-width 用户块/status、分段 Thinking、权限详情/三态选择、局部 VT 与 123 项门禁；源码入口也经 Codex PTY smoke，但真实 Windows Terminal 字体/颜色下的光标、鼠标/剪贴板粘贴、resize、Ctrl+J/历史/删除组合键、权限切换和长回答滚动仍需真人走查 |
...
```bash
npm test                              # typecheck + 123 个串联门禁脚本，必须 EXIT=0
