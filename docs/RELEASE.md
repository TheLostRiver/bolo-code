...
| 性质 | 值 |
|------|-----|
| 独立运行时依赖 | **0**（`dependencies` 恒为 `{}`；Pi TUI、宽度库与 Marked/Markdown 已内嵌进单文件） |
...
esbuild 单文件内嵌 Pi TUI 与其实际使用的宽度库；OI-14D 起同时内嵌 Marked/
Markdown。这仍然不要求用户安装 `node_modules`，但它们是随产物分发的第三方代码，
完整 MIT/BSD 文本必须随 tarball 进入 `THIRD_PARTY_NOTICES.md`。
...
包含 Pi TUI、`get-east-asian-width@1.6.0` 与 `marked@18.0.5`。MarkedJS MIT 和
John Gruber Markdown BSD 条款均已随包声明；许可真源见
...
| 指标 | 实测 | 预算（超出即需解释） |
|---|---|---|
| CLI 产物体积 | 1,834,618 bytes（OUT-3 `595b172`；较 1,385,065B 基线 +449,553B） | 基线 +1,500,000 bytes |
| CLI `--help` cold-start | OUT-3 完整门禁：增量 60.4ms | 增量 ≤ 100ms |
| retained 500 blocks / 10,000 行 | OUT-3 完整门禁：CPU 469.0ms · render heap +21.1MB · cleanup retained +1.5MB | CPU ≤ 3s · heap ≤ 128MB · cleanup ≤ 64MB |
...
| **长工具输出折叠/全文查看** | ✅ OUT-1..3 默认折叠、键盘全文与恢复完成 · ⏳ OUT-4/5 鼠标/聚合待实现 | `78ad65a` 已提供 bounded `ToolPresentation`、原始/保留规模、session-scoped spill ref 与 live event/view-state；`ab8a634` 已让成功长结果默认 summary、短结果/error 显示 bounded preview、running 显示有界 tail，并提供 `Ctrl+O` 与 retained-only `/tools`。`595b172` 已让有效 ref 按需分页读取 spill 全文，missing/corrupt/越界时明确降级，并通过 `tool_presentation` side-channel 跨 compact/resume 恢复。cleanup primitive 已有但尚无正式 session delete consumer；模型 tool message 与 plain/non-TTY 语义不变 |
...
```bash
npm test                              # 当前 typecheck + 145 个串联门禁脚本，必须 EXIT=0
