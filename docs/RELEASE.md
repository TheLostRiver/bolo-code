...
| **CLI slash 命令结果生命周期** | ⚠️ OI-15 IN PROGRESS（A/B/C/D 已关闭） | OI-15A `d681734` 已补 display policy，OI-15B `d6bd087` 已补单 panel/toast 与完整清理 primitive，OI-15C `26f796f` 已迁移只读查询到 panel/text pager；OI-15D `21ee1e2` / `87054df` 已迁移 Skills/Plugins catalog 到 stable-key OverlayHost，覆盖执行前 loading、结构化结果、`key/generation/session/cwd` stale guard、20× replace、40 项目录 18→10 行 resize、分页键与 Composer 恢复。compatibility/session-message 隔离、plain fallback、完整测试和无新增依赖均已验证；toast/error 与 normal slash compatibility cleanup 仍由 OI-15E–F 完成 |
