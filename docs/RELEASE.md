...
| **CLI slash 命令结果生命周期** | ⚠️ OI-15 OPEN | `/context`、`/skills`、`/plugins`、`/doctor` 等结果当前仍会进入 retained `compatibilityOutput` 并永久占用 Composer 上方空间；重复调用会累积。OI-15 将其拆为 Composer 下方单 panel、footer toast、OverlayHost 与显式 history，补齐 replace/TTL/input-clear/pager/stale guard；plain/non-TTY 输出保持兼容 |
