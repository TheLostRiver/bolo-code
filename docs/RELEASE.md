...
| **CLI slash 命令结果生命周期** | ⚠️ OI-15 IN PROGRESS（A/B/C 已关闭） | OI-15A `d681734` 已补 display policy，OI-15B `d6bd087` 已补单 panel/toast 与完整清理 primitive，OI-15C `26f796f` 已迁移 `/context`、`/doctor`、`/status` 与只读 help/cost/memory/mcp/hooks 到 panel/text pager；20× replace、单页/multi-page/resize/CJK、compatibility/session-message 隔离与 plain fallback 已进门禁。Skills/Plugins、toast/error 与 normal slash compatibility cleanup 仍由 OI-15D–F 完成 |
