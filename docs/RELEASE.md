...
| **CLI slash 命令结果生命周期** | ⚠️ OI-15 IN PROGRESS（A/B 已关闭） | OI-15A `d681734` 已补 display policy，OI-15B `d6bd087` 已补 Composer 下方单 panel、footer toast、generation/TTL/input/Esc/reset/restore/stop primitive；但 `/context`、`/skills`、`/plugins`、`/doctor` 等具体 consumer 尚未迁移，当前仍进入 `compatibilityOutput` 并会累积。OI-15C–F 继续接 panel/pager/OverlayHost/history 并清理 compatibility；plain/non-TTY 输出保持兼容 |
