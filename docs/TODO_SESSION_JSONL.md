...
**当前默认：** `saveSession` / autoSave **只写 jsonl**；`writeJsonSnapshot: true` 可选双写；旧 JSON 只读。
...
|------|------|------|
| T0 | 仅 JSON 快照 | 历史 |
| T1 | **双写**：jsonl append + 仍写 json 快照 | ✅ 历史 |
| T2 | resume 优先 jsonl；json 回退 | ✅ |
| T3 | 停止写 json；仅兼容读 | ✅ **当前默认** |
...
| # | 问题 | 建议默认 |
|---|------|----------|
| 1 | 是否 T1 双写？ | **已做过**；现默认 T3 只写 jsonl |
...
5. **无遥测**；无 SQLite 依赖  
6. ROADMAP M5.1 已为「JSONL transcript ✅（主路径 T3）」  
...
| 全局项 | 关系 |
|--------|------|
| M5.1 会话持久化 | 本专题主路径 **已交付**（快照 → JSONL T3） |
| M5.2 CLI | `--resume` 列表 / `--continue` 已挂 E |
| M3 Subagent | 侧链 `agent-*.jsonl` 已最小；更深 entry 见 Phase G |
| M3 MCP | **无关**；全局下一刀见 `docs/TODO.md`（SSE/HTTP · PL2） |
...
1. ~~更多 entry 类型（title…）~~ → **`title` entry + `/title` + list title ✅ 最小**  
2. ~~CLI `migrate-session` 子命令包装~~ → **`bolo --migrate-session` / `bolo migrate-session` + `--force` / `--delete-json` ✅**  
3. ~~`bolo --list`~~ → **`--list` / `-l` ✅**  
4. ~~system_note~~ → **`system_note` + `/note` + rewrite 保留 ✅ 最小**  
5. ~~lite 列表~~ → **`scanTranscriptLite` 供 list（近况 preview · R1 计数）✅ 最小**（仍全文件行扫 type，非 Buffer 快路径）  
6. Phase G 分叉链 / 完整侧链语义 ⬜  
...
**一句话：**  
JSONL 主路径（append + R1 resume + 默认停写 JSON + migrate + list）已齐；**title + system_note 日用最小**；list 走 lite 扫描；旧 JSON 只读兼容；**不上 SQLite**。
