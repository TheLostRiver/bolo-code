...
> 状态：**进行中**（A+B+C 最小 + **J-D 部分**：R1 boundary、双文件冲突、list 跟 jsonl）  
...
> **J-C+ / J-D：** 双文件 messages 优先 jsonl（**非空**）；空/全坏 jsonl **回退 JSON**；`loadTranscriptMessages` **R1**（最后 `compact_boundary` 之后）。
...
- [x] **C3** `resumeSession` 双文件策略（J-D）：  
  ...
  - 未做：停写 JSON（T3）；id 解析「先扫 jsonl 文件名再 json」与现 project→user 路径等价  
...
**验收（部分）：** jsonl 主路径绿；旧 json 仍可 resume；T2「停依赖 JSON 写」未切。
...
**验收（部分）：** 老 `.json` 不炸；新会话仍双写 json+jsonl（T1）；T3 单 jsonl 未切。
