...
> 状态：**进行中**（Phase A+B 完成；Phase C 最小：loadTranscriptMessages + JSON 缺失回退）  
...
> **T1：** `saveSession` / autoSave 双写 JSON + `.jsonl` append。  
> **J-C 起步：** `loadSession` 仍只读 JSON；JSON 缺失或路径为 `.jsonl` 时 `resumeSession` 回退 `loadTranscriptMessages`。
...
- [x] **B4** 接线（T1 最小）：  
  ...
  - [ ] `compactSession` 成功后 append `compact_boundary`（API 已有；接线后置）  
...
- [ ] **C3** `resumeSession` 完整优先链：  
  1. id → 先找 `.jsonl`  
  2. 再找 `.json`（旧）  
  3. 路径参数 `.jsonl` / `.json` 直读  
  - **最小已做：** `loadSession` 成功则不改；JSON 缺失 / 路径为 `.jsonl` 时用 `loadTranscriptMessages` 回退  
- [ ] **C4** CLI `bolo --resume` 走同一解析；摘要打印 mtime / 消息数 / 首条 user  
- [x] **C5** 测试：`scripts/test-transcript-load.ts`：写 jsonl → load messages 一致；删 JSON → resume 回退  

**验收（部分）：** jsonl messages 回退绿；旧 json 仍可 resume；完整「优先 jsonl」未切。
